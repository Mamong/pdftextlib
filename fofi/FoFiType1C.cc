...
FoFiBase(fileA, lenA, freeFileDataA)
{
	name = NULL;
	encoding = NULL;
	encodingType = FoFiType1CEncodingTypeNone;
	privateDicts = NULL;
	fdSelect = NULL;
	charset = NULL;
}
...
const char **FoFiType1C::getEncoding() {
	switch (encodingType) {
		case FoFiType1CEncodingTypeStandard:
			return fofiType1StandardEncoding;
		case FoFiType1CEncodingTypeExpert:
			return fofiType1ExpertEncoding;
		case FoFiType1CEncodingTypeUser:
...
void FoFiType1C::convertToType1(const char *psName, const char **newEncoding, GBool ascii,
	...
	enc = getEncoding();
	if (!newEncoding && enc == fofiType1StandardEncoding) {
...
void FoFiType1C::buildEncoding() {
	...
	if (topDict.encodingOffset == 0) {
		encodingType = FoFiType1CEncodingTypeStandard;
		
	} else if (topDict.encodingOffset == 1) {
		encodingType = FoFiType1CEncodingTypeExpert;
		...
		encodingType = FoFiType1CEncodingTypeUser;
	}
}
