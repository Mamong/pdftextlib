...
static int parseCharName(const char *charName, Unicode *uBuf, int uLen,
...
void GfxFont::findExtFontFile() {
  ...
  if (name) {
	...
	// Not implemented because open file is generally not allowed on iOS
	extFontFile = NULL;
}
  ...
  GfxFont(tagA, idA, nameA)
{
  ...
  for (i = 0; i < 256; ++i) {
    if (baseEncFromFontFile && baseEnc[i]) {
		encFree[i] = copyString(baseEnc[i]);
		enc[i] = encFree[i];
    } else {
		encFree[i] = NULL;
		enc[i] = baseEnc[i];
  ...
  if (obj1.isDict()) {
	...
	if (obj3.isInt()) {
	  ...
	  if (code >= 0 && code < 256) {
	    ...
	    encFree[code] = copyString(obj3.getName());
		 enc[code] = encFree[code]; 
	  }
	  ++code;
	} else {
